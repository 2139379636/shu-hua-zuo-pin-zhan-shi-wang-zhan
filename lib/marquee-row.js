...
(function(){
  ...
  function createMarqueeRow(opts){
    ...
    let touchStartX = 0;                  // touch 起始 X
    let touchMaxY = 0;                    // touch 期间 Y 的最大值（用来判下滑）
    let touchMaxX = 0;                    // touch 期间 X 的最大值（用来判横移）
    let touchMinX = 0;                    // touch 期间 X 的最小值（用来判横移）
    let touchStartTime = 0;               // touch 起始时间戳（用来判长按）
    const TOUCH_SCROLL_THRESHOLD = 10;    // 任意方向位移超过此像素 → 视为滚动意图
    ...
    const onTouchStart = (e) => {
      ...
      touchMaxX = touchStartX;
      touchMinX = touchStartX;
    ...
    const onTouchMove = (e) => {
      ...
      if (currentX > touchMaxX) touchMaxX = currentX;
      if (currentX < touchMinX) touchMinX = currentX;
    };
    ...
    const onTouchEnd = (e) => {
      ...
      // 滚动意图判定：横、纵两个方向任一超过阈值都视为滚动
      const deltaY = touchMaxY - touchStartY;
      const deltaX = Math.max(touchMaxX - touchStartX, touchStartX - touchMinX);
